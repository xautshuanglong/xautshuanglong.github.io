...
进入类定义部分后，会创建出一个新的命名空间，作为局部作用域，所有的赋值成为这个新命名空间的局部变量。
【类定义完成是（正常退出），就创建了一个类对象。】

### 类对象
类对象支持两种属性：属性引用和实例化。

类对象创建后，类命名空间中所有的命名都是有效的属性名：
...
`MyClass.i`和`MyClass.func`都是有效的属性引用，分别返回一个整数和一个方法对象。
`__doc__`也是一个有效的属性，返回类的文档字符串：`"A simple example class"`。

类的实例化使用函数符号：

	x = MyCalss()
	创建一个新的类实例并将该对象赋值给变量 x 。

将类创建为有初始状态的，需要在类中定义一个名`__init__`的特殊方法，如下：

	def __init__(self):
		self.data = []

若类定义了`__init__`方法的话，类的实例化操作会自动为新创建的类实例调用`__init__`方法。

出于弹性的需要，`__init__`方法可以有参数。例如：
	...
	x = MyClass(110,"Shuang")

### 实例对象
实例对象有两种有效的属性名：数据属性 和 方法属性。
`数据属性`相当于`C++`中的数据成员，和局部变量一样，数据属性不需要声明，第一次使用时它们就会生成。
`方法`是属于一个对象的函数。（Python中，方法不止类实例所独有，其它类型的对象也可以有方法。例如：List链表有append，insert，remove，sort等等。）
